...
ordenado <- milsa[order(milsa$Inst),]
View(ordenado)

#Será que os que tem maior instrução ganham mais?
ordernado_salario <- milsa[order(milsa$Salario),]
View(ordernado_salario)

#Os mais velhos ganham mais?
#ordenado_idade <- milsa[order(milsa$Anos),]
#View(ordenado_idade)
cor(milsa$Anos, milsa$Anos, use = "complete.obs")
